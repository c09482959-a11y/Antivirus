...
def test_stage765_malformed_running_timestamps_emit_timeout_evidence_without_aborting_sweep():
    recovery = _Recovery()
    job_records = {
        1: {
            "state": "running",
            "pid": 44,
            "running_at": "not-a-time",
            "last_heartbeat": "nan",
            "last_progress_time": "bad-progress",
            "attempt": 2,
            "stage": "raw_scan",
            "timeout_budget": {"timeout_budget": 1000.0, "heartbeat_stale_budget": 500.0, "stall_budget": 500.0},
        }
    }
    ...
    reasons = {item.get("reason") for item in evidence}
    assert "running_at_malformed" in reasons
    assert "last_heartbeat_malformed" in reasons
    assert "last_progress_time_malformed" in reasons
    for item in evidence:
        if str(item.get("reason", "")).endswith("_malformed"):
            assert item["action"] == "timeout_record_field_malformed"
...
def test_stage765_malformed_timeout_budget_values_emit_timeout_evidence_without_aborting_sweep():
    recovery = _Recovery()
    job_records = {
        1: {
            "state": "running",
            ...
            "stage": "raw_scan",
            "timeout_budget": {
                "timeout_budget": "bad-hard-budget",
                "heartbeat_stale_budget": "bad-heartbeat-budget",
                "stall_budget": "bad-stall-budget",
            },
        }
    }
    ...
    evidence = tuple(result.timeout_retry_evidence)
    reasons = {item.get("reason") for item in evidence}
    assert "timeout_budget_timeout_budget_malformed" in reasons
    assert "timeout_budget_heartbeat_stale_budget_malformed" in reasons
    assert "timeout_budget_stall_budget_malformed" in reasons
    malformed = [item for item in evidence if str(item.get("reason", "")).startswith("timeout_budget_")]
    assert malformed
    assert all(item["action"] == "timeout_budget_field_malformed" for item in malformed)
    assert all(item["final_json_must_record"] is True for item in malformed)
